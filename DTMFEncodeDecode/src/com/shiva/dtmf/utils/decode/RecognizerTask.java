package com.shiva.dtmf.utils.decode;
...
public class RecognizerTask extends AsyncTask<Void, Object, Void> {
	...
	public RecognizerTask(Controller controller,
	...
	protected Void doInBackground(Void... params) {
		while (controller.isStarted()) {
			try {
				...
				Character key = recognizer.getRecognizedKey(statelessRecognizer
						.getRecognizedKey());

				publishProgress(spectrum, key);

				// SpectrumFragment spectrumFragment = new SpectrumFragment(75,
				// 100, spectrum);
	...
	protected void onProgressUpdate(Object... progress) {
		...
		if (!key.toString().equalsIgnoreCase(lastkey.toString())) {
			//lastkey=key;
			Log.v("DTMF", "key=" + key.toString());
